...
function fft_output = intFFT(x)
    x = x'; y = m2v(x); % matrix to vector
    n = length(y);
    B1 = [1 1; 1 -1]; B2 = [1 -1i; 1 1i];
    ...
    elseif mod(n,2) == 0
        ...
        if n == 4
            fft_temp(1,:) = [fft_decompn(1,:)]; fft_temp(3,:) = [fft_decompn(2,:)]; 
        else
...
function vector_output = m2v(x)
    vector_output(1,:) = x(:); % matrix to vector
end